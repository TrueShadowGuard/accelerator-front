...
export default function checkIonicBonded(firstAminoAtoms, secondAminoAtoms) {
  ...
  function check(firstAminoAtoms, secondAminoAtoms) {
    ...
    if (
      firstAminoAtoms[0].aminoAcidId === i1 &&
      secondAminoAtoms[0].aminoAcidId === i2
    )
      debugger;

    if (
      POSITIVE_CHARGED_AMINO_ACIDS.includes(firstAminoName) &&
      NEGATIVE_CHARGED_AMINO_ACIDS.includes(secondAminoName)
    ) {
      if (
        firstAminoAtoms[0].aminoAcidId === i1 &&
        secondAminoAtoms[0].aminoAcidId === i2
